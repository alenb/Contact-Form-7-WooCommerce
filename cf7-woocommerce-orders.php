...
add_action( 'wpcf7_init', 'wpcf7_add_shortcode_orders' );

function wpcf7_add_shortcode_orders() {
	wpcf7_add_shortcode( array( 'orders', 'orders*' ),
		'wpcf7_orders_shortcode_handler', true );
}
...
add_filter( 'wpcf7_validate_orders', 'wpcf7_orders_validation_filter', 10, 2 );
add_filter( 'wpcf7_validate_orders*', 'wpcf7_orders_validation_filter', 10, 2 );

function wpcf7_orders_validation_filter( $result, $tag ) {
	$tag = new WPCF7_Shortcode( $tag );
...
if ( is_admin() ) {
	add_action( 'admin_init', 'wpcf7_add_orders_tag_generator_menu', 25 );
}

function wpcf7_add_orders_tag_generator_menu() {
	$tag_generator = WPCF7_TagGenerator::get_instance();
	$tag_generator->add( 'orders', __( 'WooCommerce Orders drop-down menu', 'contact-form-7' ),
		'wpcf7_tag_orders_generator_menu' );
}

function wpcf7_tag_orders_generator_menu( $contact_form, $args = '' ) {
	$args = wp_parse_args( $args, array() );
	$type = 'orders';

	$description = __( "Generate a form-tag for a WooCommerce Orders drop-down menu. For more details, see %s.", 'contact-form-7' );

	$desc_link = wpcf7_link( __( 'https://github.com/alenb/Contact-Form-7-WooCommerce/', 'contact-form-7' ), __( 'Contact Form 7 WooCommerce Fields', 'contact-form-7' ) );
